"""Formation autoportante - Créer un MVP avec Claude Code - Document complet pour lecture autonome"""
...
def add_flow_diagram(doc, steps, title=None):
    """Crée un schéma de flux horizontal"""
    if title:
        p = doc.add_paragraph()
    ...
    # Créer le tableau pour le diagramme
    ...
    for i, step in enumerate(steps):
        ...
        # Première et dernière en navy, autres en bleu accent
...
def add_cycle_diagram(doc, title=None):
    """Crée un schéma de cycle d'itération"""
    if title:
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = p.add_run(f"📊 {title}")
    ...
    # Créer un tableau 3x5 pour le cycle
    table = doc.add_table(rows=3, cols=5)
    table.alignment = WD_TABLE_ALIGNMENT.CENTER

    # Ligne 1: PROMPT -> GÉNÉRATION -> TEST
...
def add_vertical_flow(doc, steps, title=None):
    """Crée un schéma de flux vertical numéroté"""
    if title:
        p = doc.add_paragraph()
        run = p.add_run(f"📊 {title}")
        run.font.name = 'Arial'
    ...
    for i, (step, desc) in enumerate(steps):
        # Créer une ligne avec numéro et description
...
add_body(doc, "Ce guide vous permet de comprendre et reproduire la création d'un MVP (Minimum Viable Product) en utilisant Claude Code comme assistant de développement. Vous n'avez pas besoin de savoir programmer — Claude Code génère le code pour vous. Votre rôle est de structurer le projet et de formuler les bonnes demandes.")
...
add_table(doc, ["Section", "Contenu", "Page"],
    [["1", "Glossaire des termes techniques", "3"],
     ["2", "Le produit qu'on va construire", "4"],
     ["3", "Installation des outils", "6"],
     ["4", "La méthode Claude Code", "9"],
     ["5", "Construction pas à pas", "11"],
     ["6", "Personnaliser et faire évoluer", "18"],
     ["7", "Résoudre les problèmes", "20"],
     ["8", "Aide-mémoire des commandes", "22"]],
    col_widths=[2, 11, 3])
...
add_table(doc, ["Terme", "Définition", "Analogie"],
    [["Script", "Fichier texte contenant des instructions que l'ordinateur exécute ligne par ligne", "Une recette de cuisine"],
     ...
     ["Commit", "Sauvegarde datée du code avec un message explicatif", "Point de sauvegarde dans un jeu vidéo"],
     ["Repository (repo)", "Dossier contenant le projet et tout son historique", "Classeur avec toutes les versions"],
     ["Git", "Logiciel qui gère le versioning du code", "Le système qui gère les sauvegardes"],
     ["GitHub", "Site web qui héberge les repositories Git", "Cloud pour stocker vos classeurs"],
...
# Pipeline visuel - SCHÉMA DE FLUX
...
add_instruction(doc, "Cette section détaille l'installation de chaque outil. Suivez les étapes dans l'ordre. Comptez environ 20-30 minutes pour tout installer.")
...
doc.add_heading("3.2 Installer Git", level=2)

add_body(doc, "Git est un outil de versioning qui permet de sauvegarder l'historique du code et de le synchroniser avec GitHub.")
...
add_code(doc, [
    ...
    "",
    "4. Cliquer \"Next\" sur tous les écrans (options par défaut)",
    "",
    "5. Cliquer \"Install\" puis \"Finish\"",
])
...
add_success(doc, "Vous devez voir : git version 2.x.x")

doc.add_heading("3.3 Installer Node.js", level=2)
...
doc.add_heading("3.4 Installer Claude Code", level=2)
...
doc.add_heading("3.5 Obtenir une clé API Claude", level=2)
...
add_warning(doc, "Ne partagez JAMAIS votre clé API. Ne la mettez pas dans un fichier public sur GitHub. Elle permet d'utiliser votre compte et génère des coûts.")
...
# SCHÉMA VERTICAL - Vue d'ensemble de la construction
add_vertical_flow(doc, [
    ("Créer le projet", "Initialisation du dossier, Git et environnement Python"),
...
add_table(doc, ["Commande", "Ce qu'elle fait", "Ce que vous voyez"],
    [["mkdir C:\\Projets\\newsletter", "Crée un dossier 'newsletter'", "Rien (c'est normal)"],
     ...
     ["git init", "Initialise le versioning", "\"Initialized empty Git repository...\""],
...
add_warning(doc, "Le fichier .env contient votre clé secrète. Ne le partagez jamais et ne le mettez pas sur GitHub.")
...
1. THÈSE CENTRALE (1 paragraphe)
   ...
   Formulations type : "Ce mois confirme...", "Une rupture se dessine...", "Le marché envoie un signal clair..."

2. PREUVES ET SIGNAUX (2 paragraphes)
   Développer 2-3 dynamiques qui soutiennent la thèse.
   Faire des connexions entre les actualités, quantifier, montrer les implications de second ordre.

3. SO WHAT - IMPLICATIONS STRATÉGIQUES (1 paragraphe)
   Traduire en enjeux concrets pour un DG de banque.
   Quelles questions stratégiques ? Quels arbitrages ?

4. CONVICTION (dernière phrase, en gras)
   Format : **Notre conviction : [affirmation tranchée et mémorable].**

Le system prompt doit demander à Claude d'enrichir l'analyse avec :
- Contexte macro (taux, réglementation Bâle IV/DORA/MiCA)
- Benchmarks internationaux (banques US, UK, nordiques)
- Données de référence (ROE sectoriels, cost-to-income)""")

add_prompt_box(doc, "Créer le rédacteur - Cas terrain",
"""Ajoute une méthode generate_terrain() au module writer.py.

Cette méthode génère un mini-cas "Terrain Ares & Co" : un exemple de mission anonymisé et réaliste.

Structure stricte :
- TITRE: [5-10 mots, ex: "Optimisation du réseau d'agences d'une banque régionale"]
- PROBLÈME: [2-3 phrases décrivant la problématique client, contexte, enjeux, chiffres]
- APPROCHE: [2-3 phrases décrivant la méthodologie Ares & Co, phases, outils]
- RÉSULTATS: [2-3 phrases avec résultats quantifiés et enseignements]

Règles :
- Client toujours anonymisé ("une banque régionale", "un acteur majeur de la bancassurance")
- Chiffres crédibles et précis (%, M€, nombre de jours)
...
"""Crée le fichier src/templates/newsletter_v2.html (template Jinja2).

Design :
- Header avec dégradé bleu marine (#051E3B → #051E5B)
- Logo Ares & Co (URL fournie)
- Baseline "Cabinet de conseil de Direction Générale"

Sections :
1. ÉDITORIAL - Le texte généré, avec paragraphes stylés
2. RADAR - Les 6 articles avec : numéro, titre, source, date, résumé, lien
3. TERRAIN ARES & CO - Le cas anonymisé (fond jaune clair, bordure gauche)
4. CALL TO ACTION - Contact + bouton "Partager par email" (mailto:)

Contraintes :
- Responsive (breakpoint 600px pour mobile)
- Police Arial
- Couleur accent : #57AEE0
- Boutons arrondis (border-radius: 25px)
- Footer avec coordonnées Ares & Co""")
...
add_body(doc, "Ouvrez config/sources.yaml et ajoutez un bloc :")
...
add_code(doc, [
    "\"Ouvre src/templates/newsletter_v2.html et remplace la couleur",
...
add_table(doc, ["Commande", "Ce qu'elle vérifie"],
    [["python --version", "Python est installé et accessible"],
     ...
     ["echo %PATH%", "Affiche les chemins système"]],
    col_widths=[5, 11])
...
doc.add_heading("8.2 Commandes Git", level=2)
...
add_table(doc, ["Commande", "Description"],
    [["git init", "Initialiser un nouveau dépôt"],
     ...
     ["git commit -m \"message\"", "Sauvegarder avec un message descriptif"],
     ["git log --oneline", "Voir l'historique des commits"],
     ["git push origin main", "Envoyer les commits vers GitHub"],
     ["git pull origin main", "Récupérer les modifications depuis GitHub"]],
    col_widths=[5.5, 10.5])

doc.add_heading("8.3 Commandes Claude Code", level=2)
...
doc.add_heading("8.4 Raccourcis utiles", level=2)
...
# Save
doc.save("/home/user/genesis/Formation_Newsletter_Banking_AresCo.docx")
print("Document généré : Formation_Newsletter_Banking_AresCo.docx")
